...
def fetch_foreign_strength(stock_code: str):
    """簡化版外資力道（之後可接真實 API）"""
    # 這裡可以接 twstock 或其他外資 API
    return 0.65 # 暫時固定，之後替換成真實值
...
def main():
    print("🚀 開明體系 - 股票監控系統 最強 Phase 5.2.3 啟動")
    ...
    if results:
        ...
        telegram_summary += f"\n📈 整體信心度: {avg_conf:.1f}%\n"
        if avg_conf > 70:
            telegram_summary += "🔥 市場情緒強烈，適合積極操作"
        elif avg_conf > 55:
